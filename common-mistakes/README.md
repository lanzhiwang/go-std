...
Go is a simple and fun language, but, like any other language, it has a few gotchas... Many of those gotchas are not entirely Go's fault. Some of these mistakes are natural traps if you are coming from another language. Others are due to faulty assumptions and missing details.  Go 是一种简单而有趣的语言，但是，与任何其他语言一样，它也有一些陷阱……其中许多陷阱并不完全是 Go 的错。 如果您来自另一种语言，其中一些错误是自然陷阱。 其他是由于错误的假设和缺少细节。

A lot of these gotchas may seem obvious if you took the time to learn the language reading the official spec, wiki, mailing list discussions, many great posts and presentations by Rob Pike, and the source code. Not everybody starts the same way though and that's OK. If you are new to Go the information here will save you hours debugging your code.  如果您花时间阅读官方规范、wiki、邮件列表讨论、Rob Pike 的许多精彩帖子和演示文稿以及源代码来学习语言，那么其中很多陷阱可能看起来很明显。 不是每个人都以同样的方式开始，但这没关系。 如果您是 Go 新手，这里的信息将节省您调试代码的时间。
...
./temp.go:6:1: syntax error: unexpected semicolon or newline before {
```
...
$ go run temp.go
# command-line-arguments
./temp.go:8:9: one declared but not used
./temp.go:9:5: two declared but not used
./temp.go:10:9: three declared but not used
```
...
$ go run temp.go
# command-line-arguments
./temp.go:18:3: non-name data.result on left side of :=
./temp.go:18:16: err declared but not used
```
...
goroutine 1 [running]:
main.main()
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:5 +0x2e
exit status 2
```
...
$ go run temp.go
# command-line-arguments
./temp.go:5:9: invalid argument: m (variable of type map[string]int) for cap
```
...
$ go run temp.go
# command-line-arguments
./temp.go:4:20: cannot use nil as string value in variable declaration
./temp.go:6:13: invalid operation: x == nil (mismatched types string and untyped nil)
```
...
func main() {
    ...
    func(arr map[string]int) {
        arr["one"] = 7
        fmt.Println(arr) //prints map[one:7 two:2]
    }(m)

    fmt.Println(m) //prints map[one:7 two:2]
}
```
...
func main() {
    ...
    func(arr Point) {
        arr.x = 7
        fmt.Println(arr) //prints {7 2}
    }(m)

    fmt.Println(m) //prints {2 2}
}
```

### 13、Unexpected Values in Slice and Array "range" Clauses
...
### 14、Slices and Arrays Are One-Dimensional  切片和数组是一维的

- level: beginner

It may seem like Go supports multi-dimensional arrays and slices, but it doesn't. Creating arrays of arrays or slices of slices is possible though. For numerical computation apps that rely on dynamic multi-dimensional arrays it's far from ideal in terms of performance and complexity.  看起来 Go 似乎支持多维数组和切片，但事实并非如此。 但是，可以创建数组数组或切片切片。 对于依赖动态多维数组的数值计算应用程序，它在性能和复杂性方面远非理想。

You can build dynamic multi-dimensional arrays using raw one-dimensional arrays, slices of "independent" slices, and slices of "shared data" slices.  您可以使用原始一维数组、“独立”切片的切片和“共享数据”切片的切片来构建动态多维数组。

If you are using raw one-dimensional arrays you are responsible for indexing, bounds checking, and memory reallocations when the arrays need to grow.  如果您使用原始一维数组，您需要在数组需要增长时负责索引、边界检查和内存重新分配。

Creating a dynamic multi-dimensional array using slices of "independent" slices is a two step process. First, you have to create the outer slice. Then, you have to allocate each inner slice. The inner slices are independent of each other. You can grow and shrink them without affecting other inner slices.  使用“独立”切片的切片创建动态多维数组是一个两步过程。 首先，您必须创建外部切片。 然后，您必须分配每个内部切片。 内部切片彼此独立。 您可以在不影响其他内部切片的情况下扩展和收缩它们。
...
func main() {
    ...
    //prints: [0 1 2 3 4 5 6 7] 0xc0000b4060
...
### 15、Accessing Non-Existing Map Keys
...
$ go run temp.go
# command-line-arguments
./temp.go:7:5: cannot assign to x[0] (value of type byte)
```
...
### 17、Conversions Between Strings and Byte Slices
...
### 20、Missing Comma In Multi-Line Slice, Array, and Map Literals
...
$ go run temp.go
# command-line-arguments
./temp.go:6:18: syntax error: unexpected newline, expecting comma or }
```
...
### 21、log.Fatal and log.Panic Do More Than Log
...
```bash
$ go run temp.go
2022/05/16 15:09:24 Fatal Level: log entry
exit status 1
```

### 22、Built-in Data Structure Operations Are Not Synchronized  内置数据结构操作不同步

- level: beginner

Even though Go has a number of features to support concurrency natively, concurrency safe data collections are not one them :-) It's your responsibility to ensure the data collection updates are atomic. Goroutines and channels are the recommended way to implement those atomic operations, but you can also leverage the "sync" package if it makes sense for your application.  尽管 Go 有许多原生支持并发的特性，但并发安全的数据集合并不是其中之一。确保数据集合更新是原子的是你的责任。 Goroutines 和 channels 是实现这些原子操作的推荐方式，但如果它对您的应用程序有意义，您也可以利用“sync”包。

### 23、Iteration Values For Strings in "range" Clauses  范围子句中字符串的迭代值

- level: beginner

The index value (the first value returned by the "range" operation) is the index of the first byte for the current "character" (unicode code point/rune) returned in the second value. It's not the index for the current "character" like it's done in other languages. Note that an actual character might be represented by multiple runes. Make sure to check out the "norm" package (golang.org/x/text/unicode/norm) if you need to work with characters.  索引值（“范围”操作返回的第一个值）是第二个值中返回的当前“字符”（unicode 代码点/符文）的第一个字节的索引。 它不是当前“字符”的索引，就像在其他语言中所做的那样。 请注意，一个实际角色可能由多个符文表示。 如果您需要使用字符，请务必查看“规范”包 (golang.org/x/text/unicode/norm)。

The `for range` clauses with string variables will try to interpret the data as UTF8 text. For any byte sequences it doesn't understand it will return 0xfffd runes (aka unicode replacement characters) instead of the actual data. If you have arbitrary (non-UTF8 text) data stored in your string variables, make sure to convert them to byte slices to get all stored data as is.  带有字符串变量的 for range 子句将尝试将数据解释为 UTF8 文本。 对于它不理解的任何字节序列，它将返回 0xfffd 符文（又名 unicode 替换字符）而不是实际数据。 如果您在字符串变量中存储了任意（非 UTF8 文本）数据，请确保将它们转换为字节切片以按原样获取所有存储的数据。
...
### 24、Iterating Through a Map Using a "for range" Clause
...
### 25、Fallthrough Behavior in "switch" Statements
...
$ go run temp.go
# command-line-arguments
./temp.go:8:5: syntax error: unexpected ++, expecting }
```
...
$ go run temp.go
# command-line-arguments
./temp.go:6:17: syntax error: unexpected ~, expecting expression
```
...
### 28、Operator Precedence Differences
...
### 29、Unexported Structure Fields Are Not Encoded
...
### 30、App Exits With Active Goroutines
...
sync.runtime_Semacquire(0xc0000981b0?)
	/usr/local/go/src/runtime/sema.go:56 +0x25
sync.(*WaitGroup).Wait(0xc00008c060?)
	/usr/local/go/src/sync/waitgroup.go:136 +0x52
main.main()
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:19 +0xef
exit status 2
$
*/
...
### 31、Sending to an Unbuffered Channel Returns As Soon As the Target Receiver Is Ready
...
### 32、Sending to an Closed Channel Causes a Panic
...
main.main.func1(0x0?)
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:12 +0x31
created by main.main
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:11 +0x3d
panic: send on closed channel
...
main.main()
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:17 +0xba

goroutine 18 [chan send (nil chan)]:
main.main.func1(0x0?)
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:12 +0x31
created by main.main
	/Users/huzhi/work/code/go_code/go-std/common-mistakes/temp.go:11 +0x2a
...
### 34、Methods with Value Receivers Can't Change the Original Value
...
Old [Reddit](https://www.reddit.com/r/golang/comments/360vlb/draft_traps_gotchas_and_common_mistakes_in_go/) discussion.

Latest [Hacker News](https://news.ycombinator.com/item?id=17350776) discussion.

Thank you for your feedback and suggestions!

[golang](https://devs.cloudimmunity.com/tag/golang/index.html)

![Kyle Quest](https://devs.cloudimmunity.com/content/images/2014/12/kcq_gh_pix.png)

#### [Kyle Quest](https://devs.cloudimmunity.com/author/kyle/index.html)

Builder, breaker, and defender. Cloud, security, and Big Data.

 [Twitter](https://twitter.com/kcqon) [LinkedIn](https://www.linkedin.com/in/kylequest)

Code, Cloud, Security, and Everything in Between.

Create secure cloud apps with [Cloud Immunity](https://www.cloudimmunity.com/).