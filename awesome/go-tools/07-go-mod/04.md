...
* https://blog.csdn.net/qq_24433609/article/details/127325204

作用: 排除指定 go 包, 有 bug(或不想要)的版本

问题: 直接不引用就好, 为什么会有这个需求?

答: 我们引用多个包, 这些包中可能都包含着相同的某个包, 同时版本也很杂乱, 这个包也会间接的被引用到本项目, 但是该包有些版本有 bug, 我们不想引入, 所以采用 exclude 排除该包的 bug 版本
