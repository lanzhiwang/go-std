...
* https://blog.csdn.net/qq_24433609/article/details/127325326
...
值得一提的是, 在更新 go.sum 之前, 为了确保下载的依赖包是真实可靠的, go 命令在下载完依赖包后还会查询 `GOSUMDB 环境变量`所指示的服务器, 以得到一个权威的依赖包版本哈希值. 如果 go 命令计算出的依赖包版本哈希值与 GOSUMDB 服务器给出的哈希值不一致, go 命令将拒绝向下执行, 也不会更新 go.sum 文件.
